...
use crate::support::{Classification, classify, rule};
...
fn lifecycle_with_source_and_sink(
    source: Result<EventQuery, QueryBuildError>,
    sink: LifecycleSink,
) -> QueryDecl {
    QueryDecl::lifecycle(
...
fn lifecycle_tracks_objects_returned_by_global_calls() {
    let rule = rule("global-source").query(lifecycle_with_source_and_sink(
...
fn lifecycle_tracks_global_call_sinks() {
    let rule = rule("global-sink").query(lifecycle_with_source_and_sink(
