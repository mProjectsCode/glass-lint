...
The current review retains 8 actionable readability and maintainability issues across `glass-lint-core` and `glass-lint-project`: 4 high severity, 3 medium severity, and 1 low severity. Eleven previously completed findings were removed after verification: overlay lookup, evidence normalization, pretty-report line caching, validated project admission, loader progress/metrics ownership, project test fixtures, scope predeclaration/collection duplication, effect/projector call-payload duplication, function-summary fixed-point cloning, resolution caching outcome sharing, and core test-helper fragmentation/project coordinator organization. Seven additional findings (READ-005, READ-006, READ-007, READ-010, READ-011, READ-016, and READ-017) are now fixed and recorded in this update.
...
**Status:** Fixed. `FunctionSummary` now carries a `sinks_offset` field that tracks the number of sinks present before each propagation round, enabling delta-based processing: only sinks added since the previous round are processed. `propagate_sinks` snapshots sink offsets at round start, clones only `FactId` vectors (Copy, cheap) per caller, and delegates to `propagate_call_sinks` which borrows both target and caller parameters immutably from the table in one phase, collects projections, then applies them to the caller in a separate mutable phase. Target parameters are borrowed as `&[ParameterBinding]` and target sinks are iterated by index from the delta offset — the former `summary.parameters.clone()` and `summary.sinks().into_iter().cloned().collect::<Vec<_>>()` clones are eliminated. `collect_direct_sinks` avoids materializing a single large `Vec<(FunctionId, Vec<FactId>)>` of all call vectors, instead processing one function at a time. The round bound (64) and deterministic sink ordering via `sort_and_dedup` are preserved. Existing tests for direct, recursive, and mutually recursive helpers pass with identical evidence and exhaustion behavior.
...
**Status:** Fixed. `FlowSources` now uses `BTreeSet<SourceCandidate>` per bucket instead of `Vec`, so insertion is deduplicated and sorted by construction — the `extend` sort/dedup and `normalize` passes are removed. `extend_from_key` avoids cloning entire buckets by collecting `Copy` source candidates into a small temporary `Vec` and inserting into the destination set, reporting novelty directly. `refine_through_calls` tracks a `changed_keys` delta set across rounds; only source keys changed in the previous round trigger propagation, and unchanged keys skip `extend_from_key` entirely. `SourceBudget` now uses a direct `next_round`/`exhausted` lifecycle (no inverted exhaustion default). Unit tests cover `extend_from_key` deduplication, no-op same-key, partial novelty, deterministic ordering, budget exhaustion, and early stabilization.
...
**Status:** Fixed. The old `SummaryPath` enum (`Base(PathId)` / `Owned(Vec<PathSegment>)`) is replaced by `SummaryPathId` — a compact `Copy` handle backed by a bounded `SummaryPathInterner`. The interner stores index-addressable `(parent, segment)` nodes with a canonical edge map, answering both ID-to-node and `(parent, segment)`-to-ID lookups without sibling scans. `intern_frozen` interns artifact-frozen `PathId` values into the summary domain once, caching the mapping in `frozen_map`. `join` uses recursive call-stack traversal and edge-map lookups to concatenate paths without segment allocation. `without_first` uses call-stack reconstruction via the edge map, eliminating the `segments[1..].to_vec()` allocation. `matches_frozen` and `starts_with_frozen` compare `SummaryPathId` values against frozen `PathId` values through the cached `frozen_map` (O(1), no allocation). `owned_segments` materializes segments on demand only when the value table needs to walk a path. The interner is bounded by `MAX_SUMMARY_PATH_NODES` (1M, matching the frozen interner). All parameter paths are pre-interned during `collect_facts`, guaranteeing that `matches_frozen` and `starts_with_frozen` never need fallback allocation during projection. Tests pass for direct, recursive, and mutually recursive helpers, rest destructuring, path exhaustion, and deterministic sink ordering.
...
**Status:** Fixed. `package_occurrences` no longer allocates intermediate `Vec<Occurrence>` collections. `PackageKeyPredicate` is a concrete (non-boxed) borrowed predicate that encodes the two package-clause match kinds (`Export` and `Namespace`) with one `ModuleSpecifierPattern` and one target name check. `PackageOccurrenceIter` lazily scans base and overlay `BTreeMap` entries in deterministic order, yielding `Occurrence` values without any allocation per clause. The `CandidateOccurrences::Package` and `CandidateOccurrenceIter::Package` variants feed into the same evidence and constrained-clause paths. `OccurrenceIndex::as_map` exposes the inner map for package-scan iteration. All 385 tests, 12 e2e cases, 70 JS rules, and 98 Obsidian rules pass.

### READ-012 — Finding assembly rescans evidence and capabilities and clones report records

- **Severity:** High
- **Category:** Architecture
- **Location:** `glass-lint-core/src/lint/findings.rs:13-98`, `glass-lint-core/src/lint/linter.rs:338-395`
...
**Status:** Fixed. `by_range` now stores `usize` evidence indices instead of cloned `Evidence` DTOs; `Evidence` messages, counts, and truncation markers are constructed on the fly from the borrowed `ClassificationEvidence` data when building each `Finding`, eliminating intermediate report-record clones before the final DTO boundary. `project_findings_for_module` processes capabilities in a single pass, collecting related evidence keyed by `RuleIndex` (not `RuleId` string) alongside findings, then attaches related evidence without a second capability scan. The removed `findings_for` helper eliminates an extra iteration indirection. All 391 tests, 12 e2e cases, 70 JS rules, and 98 Obsidian rules pass.
...
**Status:** Fixed. The `matcher_families!` macro now generates `From` impls for every family with a unique type (families sharing `String` — `Imports` and `StringContains` — use `no_from` since a single `From<String>` impl would conflict). The macro also generates the `normalize()` and `validate_inner()` methods, replacing the manual match-arms in `normalization.rs` and `validation.rs` with per-family `pub(super)` functions dispatched by the macro. In `lowering.rs`, the compound functions `lower_literals`, `lower_classes_and_constructors`, and `lower_returned_members` are split into per-family functions (`lower_imports`, `lower_package_imports`, `lower_string_contains`, `lower_classes`, `lower_constructors`, `lower_returned_member_calls`, `lower_returned_member_reads`, `lower_flows`), each called directly from the compiler's exhaustive `MatcherFamily` match. A contract test (`every_family_validates_normalizes_flattens_and_compiles`) enumerates all 12 families through construction, validation, normalization, flattening, and compilation. Adding a new family now requires specifying `normalize`, `validate`, and `from`/`no_from` in the macro invocation, and a new per-family lowering function — omission from any of these paths is a compile-time error. All 461 tests, 12 e2e cases, 70 JS rules, and 98 Obsidian rules pass.
...
**Status:** Fixed. `MatcherSet` fields are now fully private; the `matcher_families!` macro accesses them within the same module, and all external construction goes through `MatcherSet::from_matchers()` and `MatcherSet::push()`. `ArgumentMatcher`, `FlowCondition`, `FlowCompletion`, `ObjectEventMatcher`, and `FlowSinkMatcher` are all struct wrappers over `pub(crate)` inner enum kinds (`ArgumentMatcherKind`, `FlowConditionKind`, `FlowCompletionKind`, `ObjectEventMatcherKind`, `FlowSinkMatcherKind`). External callers use only the public factory methods and builder APIs; direct variant construction of invalid states (empty vectors, unnormalized names) is impossible from outside the crate. Internal pattern-matching consumers access the validated inner kind via `.kind()` accessors. The `ObjectFlowMatcher` struct and `ObjectFlowMatcherBuilder` already enforced private fields and valid construction, and `ValueMatcher`/`StaticStringPredicate` were already encapsulated. All 482 tests, 12 e2e cases, 70 JS rules, and 98 Obsidian rules pass.
...
**Status:** Fixed. `walk::collect_files` is the single authoritative bounded walker owning traversal order, symlink handling (`follow_links`), excluded-directory filtering, visit/file budgets, optional deadline checks, and walk-error conversion. `walk::resolve_root` owns root-symlink metadata resolution and is used by both `SourceCorpus::discover_filtered` and `ProjectDiscovery::discover`, ensuring consistent skip-when-symlink-disabled policy. `SourceCorpus::load` and `load_source_file_at` are the single bounded source reader used by both corpus and discovery (`ProjectDiscovery::read_source` delegates directly). `SourceCorpus::collect_directory_entries` is inlined into `discover_filtered`, removing the intermediary wrapper. All 10 project tests, 12 e2e cases, 70 JS rules, and 98 Obsidian rules pass.
...
**Status:** Fixed. The `ResolutionCache` stores `Arc<ResolverOutcome>` instead of `ResolverOutcome` directly. On cache hits, `Arc::clone` (a cheap atomic reference-count bump) reuses the existing outcome; on cache misses, `Arc::new(result)` moves the resolved value into a shared handle and stores one `Arc::clone` in the cache. The session boundary receives `(*arc).clone()` — a single materialized clone at the core boundary, consistent with the session API taking ownership. `record_requests` passes the borrowed `Arc` to `enqueue_internal_target`, eliminating the independent clone for enqueuing. `ProjectResolver` already borrows validated `ProjectLoadOptions` and the cache key is the canonical `ResolutionRequestKey`; no duplicate importer/specifier representations remain.
...
**Status:** Fixed. Cache and session test cases (`cache_hit_attaches_only_current_path`, `identical_successful_source_lowers_once_then_hits`, `separate_sessions_on_one_linter_reuse_the_artifact_cache`, `session_retry_does_not_cache_parse_failure`, `session_reuses_exhausted_artifact_with_partial_status`, `rule_selection_changes_projection_without_relowering`, `all_fingerprint_dimensions_have_independent_hit_miss_tests`, `cache_eviction_is_bounded_and_deterministic`) are extracted from the coordinator `tests.rs` into the focused `tests/cache_and_session.rs` module. The coordinator retains only cross-worker, controlled-release, worker-bound, path-validation, and session-project parity tests. Integration test helpers are consolidated: `tests/support/mod.rs` now provides `assert_count_with_env` parameterized by `&Environment`, and `compact_source.rs` delegates to it instead of duplicating the linter-setup-and-count logic. `scope_precision.rs` and `semantic_matching.rs` keep local helpers because their semantics (parse-diagnostic guard, custom return types) genuinely differ. The `assert_count` helper in support delegates to `assert_count_with_env` with the default `test_environment`.
