...
pub fn rule() -> Rule {
    ...
    for root in WEB_STORAGE_ROOTS {
        for method in WEB_STORAGE_METHODS {
            builder = builder.query(EventQuery::member_call_rooted(format!("{root}.{method}")));
        }
    }
    ...
    for root in STORAGE_MANAGER_ROOTS {
        for method in STORAGE_MANAGER_METHODS {
            ...
            builder = builder.query(EventQuery::member_call_rooted(path.clone()));
    ...
    for method in COOKIE_METHODS {
        builder = builder.query(EventQuery::member_call_rooted(format!(
            "cookieStore.{method}"
        )));
    }
