...
Lint. It loads annotated JavaScript snippets, runs built-in or external tool
adapters, verifies structured findings, renders reports, and profiles folders
of JavaScript files.
...
each case, which isolates tool-global state.
...
- `profile --path PATH ...` — measure lint time across ordinary `.js` files.
