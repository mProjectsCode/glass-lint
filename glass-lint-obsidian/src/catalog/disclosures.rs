...
pub(super) fn for_rule(id: &str) -> &'static [&'static str] {
    match id {
        ...
        "metadata.events" | "lifecycle.events" | "workspace.events" => {
            &["disclosure.global_handlers_or_timers"]
        }
        "workspace.layout" | "workspace.leaf-management" => &["disclosure.workspace_layout"],
        "editor.content" | "editor.extension" | "editor.suggest" | "codemirror.extension" => {
        ...
        "bases.register" | "plugins.access" | "storage.app-data" => {
            &["disclosure.plugin_internals"]
        }
        "cli.register" | "plugins.enable-disable" | "plugins.load-unload" => {
            &["disclosure.plugin_lifecycle"]
        }
